  ...
  */
  ...
  object GenCartesianBuilders extends Template {
    ...
    def content(tv: TemplateVals) = {
      ...
      val tcBody = (synVals zip tcVals).map { case (v, t) => s"functor.map($v)(a => (a, $t))" } mkString ", "
      ...
      block"""
        ...
        |    def withImplicits[TC[_]](implicit functor: Functor[F], i0: TC[A0]) = new CartesianBuilder1(functor.map(a0)(a => (a, i0)))
        | }
        |
