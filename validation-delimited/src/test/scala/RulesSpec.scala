...
class RulesSpec extends WordSpec with Matchers {
  "Rules" should {
    "demonstrate typical usage" in {
      ...
      contactReads.validate(csv1.split(",")) shouldBe Valid(
          Contact("Ian Hummel", "ian@example.com", Some("1981-07-24")))
      contactReads.validate(csv2.split(",")) shouldBe Valid(
          Contact("Jane Doe", "jane@example.com", None))
    }

    "read optional values" in {
      val str = Array("John Doe", "", "foo", "9393.12")
      (Path \ 0).read[Delimited, String].validate(str) shouldBe Valid(
          "John Doe")
      (Path \ 1).read[Delimited, Option[String]].validate(str) shouldBe Valid(
          None)
      (Path \ 2).read[Delimited, Option[String]].validate(str) shouldBe Valid(
          Some("foo"))
    ...
    "read optional values using a different rule" in {
      ...
      (Path \ 0).read[Delimited, String].validate(str) shouldBe Valid(
          "John Doe")
      (Path \ 1)
        .read[Delimited, Option[String]](optionR(Rules.equalTo("\\N")))
        .validate(str) shouldBe Valid(None)
      (Path \ 2)
        .read[Delimited, Option[String]](optionR(Rules.equalTo("\\N")))
        .validate(str) shouldBe Valid(Some(""))
      (Path \ 3).read[Delimited, Double].validate(str) shouldBe Valid(9393.12)
    }
  }
}
